...
def entity_split(x, y, id2tag, entities, cur):
    ...
    for j in range(len(x)):
        tag = id2tag[y[j]]
        prefix = tag.split('-')[0] if '-' in tag else tag  # 'B', 'I', 'E', 'S', 'O'
        etype = tag.split('-')[1] if '-' in tag else None   # 'PER', 'LOC', 'ORG'...

        if prefix == 'B':
            start = cur + j
            entity_type = etype
        elif prefix == 'I' and start != -1 and etype == entity_type:
            continue
        elif prefix == 'E' and start != -1 and etype == entity_type:
            end = cur + j
            entities.add((start, end, entity_type))
            start, end, entity_type = -1, -1, None
        elif prefix == 'S':
            entities.add((cur + j, cur + j, etype))
            start, end, entity_type = -1, -1, None
        else:
            start, end, entity_type = -1, -1, None
...
def main(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        # 验证集评估：计算实体级别 Precision / Recall / F1
        entity_predict = set()
        entity_label = set()
        with torch.no_grad():
            model.eval()
