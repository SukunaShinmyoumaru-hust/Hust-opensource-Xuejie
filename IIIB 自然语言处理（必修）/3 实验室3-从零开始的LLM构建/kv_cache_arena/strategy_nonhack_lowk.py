...
STRATEGY_NAME = "LowKeyNormAdaptive"
MAX_BUDGET = 64
...
def _b64_adaptive(pkv, budget, step):
    ...
    if _b64_choice == "early48":
        if step < 128:
            return _lowk_abslog(pkv, budget, step, 8, 48, 2.2, 0)
        return _lowk_abslog(pkv, budget, step, 7, 30, 2.2, 0)
    if _b64_choice == "early56":
        if step < 128:
            return _lowk_abslog(pkv, budget, step, 8, 56, 2.2, 0)
        return _lowk_abslog(pkv, budget, step, 7, 30, 2.2, 0)
    ...
    return _lowk_abslog(pkv, budget, step, 7, 30, 2.2, 8)
...
def compress(past_key_values, max_budget: int, step: int):
    ...
    if max_budget <= 32:
        return _lowk_abslog(past_key_values, max_budget, step, 3, 20, 1.8, 4)
    if max_budget <= 64:
        return _b64_adaptive(past_key_values, max_budget, step)
    return _lowk_abslog(past_key_values, max_budget, step, 16, 80, 2.0, 8)
